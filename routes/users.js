...
router.post('/get_sync_data', function (req, res) {
    ...
    return sequelize.transaction(function (t) {
    	/*console.log(`INSERT INTO game_logs ("createdAt","updatedAt",device_id, sound_played, sound_selected, device_timestamp, status, score, lives)
					VALUES ${valueString}`);*/
  			return sequelize.query(`INSERT INTO game_logs ("createdAt", "updatedAt", session_id, device_id, sound_played, sound_selected, device_timestamp, status, score, lives, options_presented, cycle_count)
...
router.post('/get_letter_logs', function (req, res) {
	...
	{
	    //console.log(Item.LIVES)
	   valueString.push(`('${moment().format()}','${moment().format()}','${Item.DEVICE_ID}','${Item.SOUND_PLAYED}','${Item.TIMESTAMP}')`);
	   returnIds.push(Item.ID);
	});
    ...
    return sequelize.transaction(function (t) {
    	/*console.log(`INSERT INTO game_logs ("createdAt","updatedAt",device_id, sound_played, sound_selected, device_timestamp, status, score, lives)
					VALUES ${valueString}`);*/
  			return sequelize.query(`INSERT INTO letter_logs ("createdAt", "updatedAt", device_id, sound_played, device_timestamp)
					VALUES ${valueString}`).spread(function(results, metadata) {
  					//console.log(">>>>>>>>>>>>>>>>>>>"+results);
